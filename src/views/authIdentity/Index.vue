        ...
        >
          <template v-if="isImageField(key, val)">
            <el-image
              ...
              fit="cover"
              style="width: 200px; height: 120px; border-radius: 8px; cursor: pointer;"
            />
          </template>
          <template v-else>
...
import { getIdentityRequestApi, approveRequestApi, rejectRequestApi } from "@/api/authIdentity";
...
const fetchRequests = async () => {
  try {
    const res = await getIdentityRequestApi({});
